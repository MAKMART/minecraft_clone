...
void camera_pose_system(ECS& ecs, Entity player, const frame_context& ctx, float dt, const InputManager& input)
{
  ...
  InputComponent* input_comp = ecs.get_component<InputComponent>(player);

  if (!trans || !input_comp || !target_trans)
  ...
  if (input.isMouseTrackingEnabled()) {
    camera_model::apply_mouse_look(input_comp->mouse_delta, angles->sensitivity, angles->yaw, angles->pitch);
  }
