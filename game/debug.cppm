...
#undef X
  ...
  void setup_imgui(game_state& g_state, const frame_context& frame_ctx, const InputManager& input, const ChunkManager& manager, CameraController* ctrl, const PlayerState* player_state) {
    glm::vec3 pos = g_state.ecs.get_component<Transform>(g_state.player.self)->pos;
    glm::vec3 vel = g_state.ecs.get_component<Velocity>(g_state.player.self)->value;
    glm::ivec3 chunkCoords = world_to_chunk(pos);
    glm::ivec3 localCoords = world_to_local(pos);
    if (ImGui::Begin("Console")) {
      std::vector<std::string> local_copy;

      {
        std::scoped_lock lock(log::imgui_mutex);
        local_copy = log::imgui_buffer;
      }
      ImGuiListClipper clipper;
      clipper.Begin(local_copy.size());

      while (clipper.Step())
      {
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
        {
          ImGui::TextUnformatted(local_copy[i].c_str());
        }
      }
    }
    ImGui::End();
    if (ImGui::Begin("DEBUG", NULL,
      ...
      ImGui::Text("Player STATE: %s", player_mode_to_string(*g_state.ecs.get_component<PlayerMode>(g_state.player.self)).data());
      ImGui::Text("Player STATE: %s", player_state_to_string(*g_state.ecs.get_component<PlayerState>(g_state.player.self)).data());
      ...
      ImGui::Text("%s", Block::toString(static_cast<Block::blocks>(g_state.player.selectedBlock)));
      ...
      ImGui::PopFont();
      ImGui::Indent();
      DrawBool("is OnGround", g_state.ecs.get_component<Collider>(g_state.player.self)->is_on_ground);
      DrawBool("is Damageable", g_state.player.isDamageable);
      ...
      DrawBool("Player can place blocks", g_state.player.canPlaceBlocks);
      DrawBool("Player can break blocks", g_state.player.canBreakBlocks);
      DrawBool("renderSkin", g_state.player.renderSkin);
      ImGui::Unindent();
      ImGui::Spacing();
      ImGui::PushFont(ImGui::GetFont()); // Or use a bold/large font if you have one
      ...
      glm::vec3& camPos = g_state.ecs.get_component<Transform>(frame_ctx.active_camera)->pos;
      ImGui::Text("Camera position: %f, %f, %f", camPos.x, camPos.y, camPos.z);
      ImGui::Unindent();
      if (!input.isMouseTrackingEnabled()) {
        if (ImGui::CollapsingHeader("Settings")) {
          if (ImGui::TreeNode("Player")) {
            auto* cfg = g_state.ecs.get_component<MovementConfig>(g_state.player.self);
            ...
            ImGui::SliderFloat("Max Interaction Distance", &g_state.player.max_interaction_distance, 0.0f, 1000.0f);
            ImGui::TreePop();
          }
          if (ImGui::TreeNode("Camera")) {
            // ImGui::SliderFloat("Mouse Sensitivity", &ctrl->sensitivity, 0.01f, 2.0f);
            ImGui::SliderFloat("FOV", &frame_ctx.cam->fov, 0.001f, 179.899f);
            ImGui::SliderFloat("Near Plane", &frame_ctx.cam->near_plane, 0.001f, 10.0f);
            ImGui::SliderFloat("Far Plane", &frame_ctx.cam->far_plane, 10.0f, 1000000.0f);
          ...
          if (ImGui::TreeNode("Miscellaneous")) {
            ...
            ImGui::Checkbox("render_terrain: ", &g_state.render_terrain);
            ImGui::Checkbox("render_player: ", &g_state.player.renderSkin);
