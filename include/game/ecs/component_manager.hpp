...
public:
	template<typename T>
		void add_component(Entity e, const T& component) { get_storage<T>().add(e, component); }

	template<typename T>
		bool has_component(Entity e) { return std::get<ComponentStorage<T>>(storages).has(e); }


	template<typename T>
		T* get_component(Entity e) { return get_storage<T>().get(e); }
	...
	template<typename T>
		void remove_component(Entity e) { get_storage<T>().remove(e); }
