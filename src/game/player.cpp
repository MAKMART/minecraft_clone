...
void Player::placeBlock(ChunkManager& chunkManager)
{
	...
	if (hitResult.has_value()) {
		...
		if (chunkManager.getChunk(placePos)->get_block(localBlockPos.x, localBlockPos.y, localBlockPos.z).type != Block::blocks::AIR) {
			log::system_info("Player", "❌ Target block is NOT air! It's type: {}", Block::toString(chunkManager.getChunk(placePos)->get_block(localBlockPos.x, localBlockPos.y, localBlockPos.z).type));
			return;
		}
