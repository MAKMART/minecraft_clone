...
//#if defined(DEBUG)
	...
	// logger::system_info("UI", "Rml::Debugger::Initialise should've run");
//#endif
	if (!Rml::LoadFontFace(fontPath.string())) {
		logger::system_error("UI", "Failed to load font: {}", fontPath.string());
	}
...
Rml::ElementDocument* UI::LoadDocument(const std::string& name, const std::filesystem::path& path) noexcept
{
    ...
    if (!doc) {
        logger::system_error("UI", "Failed to load document: {}", path.string());
        return nullptr;
    }
    documents[name] = doc;
    return doc;
...
int UI::GetKeyModifiers(const InputManager& input) noexcept
{
	int modifiers = 0;
...
void UI::RenderGeometry(Rml::CompiledGeometryHandle handle, Rml::Vector2f translation, Rml::TextureHandle texture)
{
	...
	if (texture != 0) {
		...
		if (tex != texture_map.end()) {
			...
			logger::system_error("UI", "Invalid texture handle {}", texture);
			return;
		}
...
Rml::TextureHandle UI::LoadTexture(Rml::Vector2i& out_dimensions, const Rml::String& source)
{
	...
	*/
	logger::system_info("UI", "Path: {}", source_path.string());
	...
	logger::system_info("UI", "Loaded texture handle {} from {}", handle, real_source.string());
	return handle;
}
...
Rml::TextureHandle UI::GenerateTexture(Rml::Span<const Rml::byte> source, Rml::Vector2i source_dimensions)
{
	...
	logger::system_info("UI", "Generated texture handle {}", handle);
	return handle;
}
...
void UI::ReleaseTexture(Rml::TextureHandle handle)
{
	...
	if (it != texture_map.end()) {
		logger::system_info("UI", "Releasing texture handle {}", handle);
		texture_map.erase(it);
	} else {
		logger::system_error("UI", "Warning: Attempt to release invalid texture handle {}", handle);
	}
}
...
Rml::LayerHandle UI::PushLayer()
{
	...
	{
		logger::system_error("UI", "Framebuffer incomplete when pushing layer");
...
Rml::CompiledFilterHandle UI::CompileFilter(const Rml::String& name, const Rml::Dictionary& parameters)
{
    ...
    logger::system_info("UI", "CompileFilter called for '{}', not supported", name);
    return 0;
}
...
Rml::CompiledShaderHandle UI::CompileShader(const Rml::String& name, const Rml::Dictionary& parameters)
{
	...
	(void)parameters;
	logger::system_info("UI", "CompileShader called for '{}', not supported", name);
    return 0;
}
